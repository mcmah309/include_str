use include_str::include_sql_str;

#[test]
fn compacts_a_complete_cte_without_changing_literals_or_parameters() {
    const SQL: &str = include_sql_str!("fixtures/report.sql");
    assert_eq!(
        SQL,
...
fn preserves_a_complete_procedure_body_byte_for_byte() {
    const SQL: &str = include_sql_str!("fixtures/procedure.sql");
    assert_eq!(
        SQL,
...
fn preserves_json_multiline_unicode_and_escaped_identifier_delimiters() {
    const SQL: &str = include_sql_str!("fixtures/quoted.sql");
    assert_eq!(
        SQL,
